...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
    cat <<EOF
Usage:
  bash train_once_anchor3dlane_r50x2_origsplit.sh [checkpoint_path] [extra train args...]

Examples:
  bash train_once_anchor3dlane_r50x2_origsplit.sh
  bash train_once_anchor3dlane_r50x2_origsplit.sh /path/to/latest.pth
  GPU_IDS=0,1,2,3 NPROC_PER_NODE=4 bash train_once_anchor3dlane_r50x2_origsplit.sh /path/to/latest.pth --epochs 80
EOF
    exit 0
fi
