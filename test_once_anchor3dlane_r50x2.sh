...
CHECKPOINT_PATH="${1:-${REPO_ROOT}/work_dirs/once_3dlanes_anchor_r50x2/latest.pth}"
GPU_IDS="${GPU_IDS:-0}"
...
usage() {
    cat <<EOF
Usage:
  bash test_once_anchor3dlane_r50x2.sh [checkpoint_path]

Defaults:
  checkpoint_path = ${REPO_ROOT}/work_dirs/once_3dlanes_anchor_r50x2/latest.pth
...
Optional environment variables:
  ...
  CONFIG_PATH     Validation config. Default: tools/once_anchor3dlane_r50x2_config.py
...
PRED_ROOT="${PRED_ROOT:-${REPO_ROOT}/work_dirs/once_3dlanes_anchor_r50x2/predictions/${CHECKPOINT_STEM}}"
