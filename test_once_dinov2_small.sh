...
CHECKPOINT_PATH="${1:-${REPO_ROOT}/work_dirs/once_3dlanes_dinov2_small/latest.pth}"
GPU_IDS="${GPU_IDS:-0}"
...
usage() {
    cat <<EOF
Usage:
  bash test_once_dinov2_small.sh [checkpoint_path]

Defaults:
  checkpoint_path = ${REPO_ROOT}/work_dirs/once_3dlanes_dinov2_small/latest.pth
...
Optional environment variables:
  ...
  CONFIG_PATH     Validation config. Default: tools/once_dinov2_small_config.py
...
PRED_ROOT="${PRED_ROOT:-${REPO_ROOT}/work_dirs/once_3dlanes_dinov2_small/predictions/${CHECKPOINT_STEM}}"
