...
train_gt_root = os.getenv("ONCE_3DLANES_SEQ_TRAIN_GT_ROOT", os.path.join(dataset_root, "train"))
val_gt_root = os.getenv("ONCE_3DLANES_SEQ_VAL_GT_ROOT", train_gt_root)
base_index_file = os.getenv("ONCE_3DLANES_SEQ_BASE_INDEX", os.path.join(dataset_root, "list", "train.txt"))
...
model_save_path = os.getenv(
    "ONCE_3DLANES_WORKDIR",
    os.path.join(repo_root, "work_dirs", "once_3dlanes_sequence_split"),
)
...
default_backbone_ckpt = os.path.join(repo_root, "ckpt", "resnet34-b627a593.pth")
...
val_ratio_th = float(os.getenv("ONCE_3DLANES_VAL_RATIO_TH", "0.6"))
val_dist_th = float(os.getenv("ONCE_3DLANES_VAL_DIST_TH", "1.5"))
...
backbone_pretrained = os.getenv("ONCE_3DLANES_BACKBONE_PRETRAINED", "1") == "1"
backbone_pretrained_path = os.getenv("ONCE_3DLANES_BACKBONE_CKPT", default_backbone_ckpt)
