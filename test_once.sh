...
CONFIG_PATH="${CONFIG_PATH:-${REPO_ROOT}/tools/once_config.py}"
CHECKPOINT_PATH="${1:-${REPO_ROOT}/ep0099.pth}"
...
usage() {
    cat <<EOF
Usage:
  bash test_once.sh [checkpoint_path]

Defaults:
  checkpoint_path = ${REPO_ROOT}/ep0099.pth
...
Optional environment variables:
  ...
  CONFIG_PATH     Validation config. Default: tools/once_config.py
  GPU_IDS         CUDA_VISIBLE_DEVICES value for inference
  RATIO_TH        Ratio threshold for eval_once_with_ratio.py
  PRED_ROOT       Prediction output directory
  GT_ROOT         Override GT root for validation/evaluation
...
PRED_ROOT="${PRED_ROOT:-${REPO_ROOT}/work_dirs/once_3dlanes/predictions/${CHECKPOINT_STEM}}"
...
echo "[1/2] Running inference with checkpoint: ${CHECKPOINT_PATH}"
python tools/val_once.py "${VAL_ARGS[@]}"

echo "[2/2] Running ratio metric + ONCE official benchmark"
python tools/eval_once_with_ratio.py "${EVAL_ARGS[@]}"
